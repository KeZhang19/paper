...
const categories = [
  ...
  { id: "tactile-simulation", label: { zh: "触觉仿真", en: "Tactile Simulation" } },
...
const papers = [
  ...
  {
    id: "rlt-vla-online-rl",
    categories: ["real-robot-rl"],
    pdf: "https://www.pi.website/download/rlt.pdf",
    project: "https://www.pi.website/research/rlt",
    arxiv: "https://arxiv.org/abs/2604.23073",
    year: "2026",
    venue: "arXiv:2604.23073",
    zh: {
      ...
      status: "已整理",
      tags: ["Real-World RL", "VLA Fine-tuning", "RL Token"],
      mainContent:
        "RLT 研究如何让预训练 VLA 在真实机器人上用少量在线 RL 快速提升精密操作能力。方法冻结 VLA 主体，只训练一个能从 VLA 内部特征压缩出任务相关状态的 RL token，再用小型 actor-critic 在这个 token 上学习，对 VLA 生成的动作 chunk 做局部 refinement。",
      innovations: [
        "提出 RL token 作为 VLA 与在线 RL 之间的低维接口，避免直接对大规模 VLA 做昂贵的端到端 RL 微调。",
        "把 VLA 的广泛感知和任务先验保留下来，让轻量 actor-critic 只负责最困难、最精密的任务阶段。",
        "策略学习 action chunks 并以 VLA reference action 为条件，同时用行为正则保持探索靠近原始 VLA 行为。",
        "在螺丝安装、扎带固定、以太网插入和电源线插入等真实机器人任务上，关键阶段速度最高提升约 3 倍，并在部分任务超过专家遥操作速度。",
      ],
      implementation: [
        "先给 VLA 加入 encoder-decoder transformer，使其输出能重建内部表征的 compact RL token。",
        "在线阶段冻结 VLA 和 RL token 模块，用 RL token、语言/视觉状态和 VLA reference action 训练小型 actor-critic。",
        "RL actor 输出 action chunk refinement，并通过 reference-action conditioning、regularization 和 dropout 提升样本效率。",
        "训练只作用于任务的 contact-critical phase，前序抓取和搬运动作仍由基础 VLA 完成。",
      ],
    },
    en: {
      title: "RL Token: Bootstrapping Online RL with Vision-Language-Action Models",
      ...
      tags: ["Real-World RL", "VLA Fine-tuning", "RL Token"],
      mainContent:
        "RLT studies how pretrained VLA models can rapidly improve precise manipulation skills through a small amount of online real-robot RL. The method freezes the main VLA, trains an RL token that compresses task-relevant internal VLA features, and then trains a small actor-critic on this token to locally refine the VLA's action chunks.",
      innovations: [
        "It introduces the RL token as a low-dimensional interface between a VLA and online RL, avoiding expensive end-to-end RL fine-tuning of the full VLA.",
        "The broad perception and task priors of the VLA are preserved, while a lightweight actor-critic focuses only on the hardest high-precision task phases.",
        "The policy learns action chunks conditioned on the VLA reference action, with behavioral regularization keeping exploration close to the base VLA.",
        "On real-robot screw installation, zip tie fastening, Ethernet insertion, and power insertion, RLT improves critical-phase speed by up to about 3x and can exceed expert teleoperation speed on some tasks.",
      ],
      implementation: [
        "An encoder-decoder transformer is first added to the VLA so it exposes a compact RL token that can reconstruct internal representations.",
        "During online training, the VLA and token module are frozen; a small actor-critic is trained from the RL token, observations, and VLA reference action.",
        "The RL actor outputs action-chunk refinements, using reference-action conditioning, regularization, and dropout for sample efficiency.",
        "Training targets only the contact-critical phase of each task, while the base VLA handles earlier grasping and transport phases.",
      ],
    },
  },
  {
    id: "tacsl",
    categories: ["tactile-simulation"],
    pdf: "https://arxiv.org/pdf/2408.06506",
    project: "https://iakinola23.github.io/tacsl/",
    arxiv: "https://arxiv.org/abs/2408.06506",
    year: "2025",
    venue: "IEEE T-RO 2025 / arXiv:2408.06506",
    zh: {
      ...
      status: "已整理",
      tags: ["Tactile Simulation", "Visuotactile Sensing", "Sim-to-Real"],
      mainContent:
        "TacSL 研究如何在机器人仿真中高效生成视觉触觉传感器信号，并用这些触觉观测训练可迁移到真实世界的接触操作策略。它把 GelSight 类视觉触觉传感器建模、接触力场提取、触觉图像生成、操作环境和策略学习工具整合到 Isaac Gym/Isaac Simulator 生态中，使触觉策略可以通过大规模仿真数据学习，再零样本部署到真实 peg placement 和 peg insertion 任务。",
      innovations: [
        "提出 GPU-based visuotactile sensor simulation library，可同时生成触觉 RGB 图像和 normal/shear force fields，并比 prior state-of-the-art 快 200 倍以上。",
        "把触觉仿真模块直接集成到通用机器人仿真器中，支持大规模并行环境，而不是离线或低速的单独触觉渲染流程。",
        "提供触觉学习 toolkit，包括多种 GelSight 传感器模型、接触丰富装配环境、offline/online distillation 和 RL 算法。",
        "提出 asymmetric actor-critic distillation (AACD)，利用带特权状态的 critic 加速高维触觉图像策略学习。",
        "系统评估 state、vision、tactile 和 multimodal inputs 在接触操作中的作用，并展示触觉策略零样本 sim-to-real transfer。",
      ],
      implementation: [
        "仿真模块用刚体接触近似估计弹性体表面变形，再将变形和接触信息渲染成视觉触觉图像，同时输出可用于分析和学习的接触力分布。",
        "学习环境覆盖 peg placement、peg insertion 等接触密集任务，并随机化目标位置、peg-in-hand 位姿和传感器相关条件。",
        "训练流程先学习 state-based teacher 或 critic，再通过 AACD/蒸馏让使用触觉图像、视觉或多模态输入的 student policy 获得可执行策略。",
        "为真实迁移加入 soft-contact parameter randomization、触觉图像增强、当前图像与 nominal image 的差分/拼接表示等策略。",
        "真实 Franka + GelSight Finger 实验中，peg placement 和 peg insertion 策略均从仿真零样本部署，其中 insertion 在 81 次试验中达到 82.7% 成功率。",
      ],
    },
    en: {
      title: "TacSL: A Library for Visuotactile Sensor Simulation and Learning",
      ...
      tags: ["Tactile Simulation", "Visuotactile Sensing", "Sim-to-Real"],
      mainContent:
        "TacSL studies how to efficiently generate visuotactile sensor signals in robot simulation and use those tactile observations to train contact-rich policies that transfer to the real world. It integrates GelSight-style sensor modeling, contact-force extraction, tactile image generation, manipulation environments, and policy-learning tools into the Isaac Gym/Isaac Simulator ecosystem, enabling large-scale tactile policy learning and zero-shot deployment on real peg placement and peg insertion tasks.",
      innovations: [
        "It introduces a GPU-based visuotactile sensor simulation library that generates tactile RGB images plus normal and shear force fields over 200x faster than prior state-of-the-art methods.",
        "The tactile simulator is integrated into a general-purpose robotics simulator, supporting massively parallel environments instead of a slow standalone rendering pipeline.",
        "TacSL provides a learning toolkit with GelSight sensor models, contact-intensive assembly environments, offline/online distillation, and RL algorithms.",
        "The paper proposes asymmetric actor-critic distillation (AACD), using a privileged-state critic to accelerate policy learning from high-dimensional tactile images.",
        "It systematically compares state, vision, tactile, and multimodal inputs for contact-rich manipulation and demonstrates zero-shot tactile sim-to-real transfer.",
      ],
      implementation: [
        "The simulation module approximates elastomer deformation from rigid-body contacts, renders the resulting tactile images, and also exposes contact-force distributions for analysis and learning.",
        "The learning environments cover contact-heavy tasks such as peg placement and peg insertion, with randomized target locations, peg-in-hand poses, and sensor conditions.",
        "Training first learns a state-based teacher or critic, then uses AACD/distillation so student policies can act from tactile images, vision, or multimodal observations.",
        "For real transfer, the method uses soft-contact parameter randomization, tactile image augmentation, and current-versus-nominal image difference or concatenation representations.",
        "On a real Franka robot with a GelSight Finger, simulation-trained policies transfer zero-shot; the peg insertion policy succeeds in 67 of 81 trials, reaching 82.7% success.",
      ],
    },
  },
  {
    id: "tacex",
    categories: ["tactile-simulation"],
    pdf: "https://arxiv.org/pdf/2411.04776",
    project: "https://sites.google.com/view/tacex",
    arxiv: "https://arxiv.org/abs/2411.04776",
    year: "2024",
    venue: "CoRL Workshop 2024 / arXiv:2411.04776",
    zh: {
      ...
      status: "已整理",
      tags: ["Tactile Simulation", "GelSight", "Isaac Sim"],
      mainContent:
        "TacEx 研究如何在 Isaac Sim / Isaac Lab 中构建更可靠、易扩展的 GelSight 视觉触觉仿真框架。它把软体接触物理仿真、Taxim 光学触觉图像生成和 FOTS marker motion simulation 组合起来，目标是在接触丰富操作任务中同时得到 gel deformation、GelSight RGB 图像和 marker displacement 等高维触觉观测，并让这些观测能直接用于 RL 环境。",
      innovations: [
        "提出模块化 TacEx 框架，把 GelSight Mini 触觉仿真接入 Isaac Sim 和 Isaac Lab，便于和现有机器人环境、渲染、ROS 与 RL 工具链结合。",
        "同时比较 rigid compliant contact、PhysX soft body 和 GIPC soft-body contact 三种物理仿真路径，为不同精度/速度需求提供选择。",
        "使用 GIPC 处理 FEM-like soft-body contact，支持更稳定的软体大变形、摩擦和 soft-to-soft contact，比单纯刚体近似更适合触觉接触。",
        "复用 Taxim 生成触觉 RGB 图像、复用 FOTS 生成 marker motion field，使物理仿真、光学仿真和 marker 仿真可以解耦替换。",
        "在 Isaac Lab 中实现 object pushing、object lifting、pole balancing 等 RL 环境，验证触觉高维观测可以接入训练流程。",
      ],
      implementation: [
        "Isaac Sim 负责场景、机器人运动和渲染；GelSight 外壳随机器人运动，gelpad 的未固定顶点由 GIPC solver 更新，实现软体形变和接触交互。",
        "触觉图像生成先通过 Isaac Sim camera 得到 height map，再用 Gaussian smoothing 和 Taxim 的 surface-normal 到 RGB 映射生成 tactile RGB。",
        "marker simulation 使用 FOTS，根据 height map、接触中心和物体相对 gelpad 的运动估计 marker displacement field。",
        "实验包含 ball rolling、object lifting、beam twisting 等仿真 showcase，并测量 Taxim/FOTS、PhysX 与 GIPC 等模块的运行速度。",
        "RL 部分用 PPO 在 Isaac Lab 环境中测试 tactile observations；论文也明确指出当前主要是仿真定性验证，尚缺少真实 Sim2Real 和更系统的定量比较。",
      ],
    },
    en: {
      title: "TacEx: GelSight Tactile Simulation in Isaac Sim -- Combining Soft-Body and Visuotactile Simulators",
      ...
      tags: ["Tactile Simulation", "GelSight", "Isaac Sim"],
      mainContent:
        "TacEx studies how to build a reliable, extensible GelSight tactile simulation framework inside Isaac Sim and Isaac Lab. It combines soft-body contact physics, Taxim optical tactile image generation, and FOTS marker-motion simulation so contact-rich manipulation environments can expose gel deformation, GelSight RGB images, marker displacement fields, and other high-dimensional tactile observations directly to RL pipelines.",
      innovations: [
        "It introduces a modular TacEx framework that brings GelSight Mini tactile simulation into Isaac Sim and Isaac Lab, making it compatible with existing robot environments, rendering, ROS, and RL tooling.",
        "The paper compares three physics paths: rigid compliant contact, PhysX soft bodies, and GIPC soft-body contact, giving users different accuracy and speed tradeoffs.",
        "GIPC provides FEM-like soft-body contact with stable large deformation, friction, and soft-to-soft contacts, making it more physically plausible than pure rigid-body approximations.",
        "Taxim is reused for tactile RGB rendering and FOTS for marker-motion fields, decoupling physics, optical, and marker simulation components.",
        "The authors implement Isaac Lab RL environments such as object pushing, object lifting, and pole balancing to validate that high-dimensional tactile observations can enter the training loop.",
      ],
      implementation: [
        "Isaac Sim handles scene setup, robot motion, and rendering; the GelSight case follows the robot, while free gelpad vertices are updated by a GIPC solver for soft deformation and contact interaction.",
        "Tactile image generation first creates height maps using Isaac Sim cameras, smooths them, and then maps surface normals to RGB values with the Taxim approach.",
        "Marker simulation uses FOTS, estimating marker displacement fields from height maps, contact centers, and object motion relative to the gelpad.",
        "Experiments include ball rolling, object lifting, and beam twisting simulation showcases, along with runtime measurements for Taxim/FOTS, PhysX, and GIPC modules.",
        "The RL validation uses PPO in Isaac Lab with tactile observations; the paper explicitly notes that the current work is mainly simulation-based qualitative validation and lacks real-world sim-to-real experiments.",
      ],
    },
  },
  {
    ...
    project: "https://arxiv.org/abs/2602.21625",
    arxiv: "https://arxiv.org/abs/2602.21625",
    year: "2026",
    venue: "arXiv:2602.21625",
    zh: {
      ...
      status: "已整理",
      tags: ["Tactile Simulation", "Sim-to-Real", "Deform Map"],
      mainContent:
        "Tacmap 研究视觉触觉传感器的 sim-to-real gap：真实 GelSight/DIGIT 类传感器图像包含复杂光学、弹性体形变和硬件差异，而高保真 FEM 又太慢，不适合大规模 RL。论文提出用 geometry-consistent penetration depth map 作为仿真和真实之间的共同触觉表示，在仿真中直接计算物体与弹性体的 3D 穿透深度，在真实世界中把 raw tactile image 映射到同样的 deform map，从而让策略看到更一致、物理可解释的触觉几何。",
      innovations: [
        "把触觉 sim-to-real 对齐从 raw tactile image 空间转移到统一的 geometric deform map 空间，降低光照、反射和相机外观带来的 domain shift。",
        "用 penetration depth 近似弹性体形变，避免 FEM 的高计算成本，同时比简单 depth-buffer projection 更有几何和接触物理一致性。",
        "提出 generalized normal-projection rendering pipeline，可支持平面传感器和曲面 fingertip，减少传统平面触觉仿真在曲面上的投影失真。",
        "真实侧使用自动化数据采集平台生成 raw tactile image 与 ground-truth deform map 配对数据，再训练 ResNet encoder-decoder 做 image-to-deform translation。",
        "在 SharpaWave 灵巧手上验证仅用仿真训练的触觉 in-hand rotation policy 可以零样本迁移到真实机器人。",
      ],
      implementation: [
        "仿真中定义 undeformed sensor surface 和外侧 virtual sensing surface，在 sensing surface 上离散采样，并沿局部法线做 ray casting 计算物体穿透深度。",
        "Tacmap 同时提供 net force、contact position 和 dense deform map：仿真侧从物理引擎/contact sensor 获取力和接触位置，真实侧用触觉图像网络估计力，并从 deform map 中求接触区域质心。",
        "真实数据采集使用高精度三轴运动平台驱动多种几何压头与传感器接触，记录压头位姿，并用与仿真一致的几何逻辑生成 deform map 标签。",
        "工具集集成到 Isaac Lab 和 MuJoCo：Isaac Lab 使用 GPU Raycaster API 支持上千并行环境，MuJoCo 中用 mj ray 查询穿透深度。",
        "实验比较方形/圆柱压头下的力、接触位置和 deform map 一致性，并用 PPO 在仿真中训练基于 Tacmap stream 的球体手内旋转策略，再直接部署到真实 SharpaWave 手。",
      ],
    },
    en: {
      title: "Tacmap: Bridging the Tactile Sim-to-Real Gap via Geometry-Consistent Penetration Depth Map",
      ...
      tags: ["Tactile Simulation", "Sim-to-Real", "Deform Map"],
      mainContent:
        "Tacmap addresses the sim-to-real gap for vision-based tactile sensors. Real GelSight/DIGIT-style images contain complex optics, elastomer deformation, and hardware-specific artifacts, while high-fidelity FEM is too slow for large-scale RL. The paper proposes a geometry-consistent penetration depth map as a shared tactile representation: simulation computes 3D object-elastomer penetration directly, while the real system maps raw tactile images into the same deform-map space, giving policies a more consistent and physically interpretable tactile geometry.",
      innovations: [
        "It shifts tactile sim-to-real alignment from raw tactile images into a unified geometric deform-map space, reducing domain shift from lighting, reflection, and camera appearance.",
        "Penetration depth approximates elastomer deformation without the cost of FEM while preserving more contact-geometry consistency than simple depth-buffer projections.",
        "A generalized normal-projection rendering pipeline supports both flat sensors and curved fingertips, reducing projection artifacts in non-planar tactile geometries.",
        "The real-world side uses an automated data-collection rig to create paired raw tactile images and ground-truth deform maps, then trains a ResNet encoder-decoder for image-to-deform translation.",
        "On a SharpaWave dexterous hand, the paper demonstrates zero-shot transfer of a tactile in-hand rotation policy trained only in simulation.",
      ],
      implementation: [
        "In simulation, the method defines an undeformed sensor surface and an external virtual sensing surface, samples the sensing surface, and ray-casts along local normals to compute object penetration depth.",
        "Tacmap exposes net force, contact position, and dense deform maps: simulation reads force and contact positions from the physics engine, while the real system estimates force from tactile images and derives contact centroids from deform maps.",
        "Real data is collected with a high-precision three-axis motion stage pressing diverse geometric indenters into the sensor; indenter poses are recorded and converted into deform-map labels using the same geometry logic as simulation.",
        "The toolkit is integrated into Isaac Lab and MuJoCo: Isaac Lab uses the GPU Raycaster API for thousands of parallel environments, while MuJoCo uses mj ray to query penetration depths.",
        "Experiments compare force, contact position, and deform-map consistency for square and cylindrical indenters, then train a PPO in-hand ball rotation policy with Tacmap streams in simulation and deploy it directly on the real SharpaWave hand.",
      ],
    },
  },
  {
    id: "univtac",
    categories: ["tactile-simulation"],
    pdf: "https://arxiv.org/pdf/2602.10093",
    project: "https://univtac.github.io/",
    arxiv: "https://arxiv.org/abs/2602.10093",
    year: "2026",
    venue: "arXiv:2602.10093",
    zh: {
      ...
      status: "已整理",
      tags: ["Tactile Simulation", "Visuotactile Benchmark", "Representation Learning"],
      mainContent:
        "UniVTAC 研究如何用统一的仿真平台解决视觉触觉机器人学习中的数据稀缺和评测不统一问题。平台支持 GelSight Mini、ViTai GF225 和 Xense WS 三类常见视觉触觉/力触觉传感器，可自动生成 RGB、深度、触觉和动作数据；在此基础上，论文训练 UniVTAC Encoder 学习以触觉为中心的视觉触觉表示，并提出包含 8 个接触丰富操作任务的 UniVTAC Benchmark，用于评估触觉驱动策略。",
      innovations: [
        "提出统一视觉触觉仿真平台，覆盖多种传感器形态和接触任务，降低触觉数据采集与 benchmark 搭建成本。",
        "通过自动化操作 API 生成可控接触交互，包括 grasp、move、place、probe 和 rotate，便于大规模合成多模态数据。",
        "提出 UniVTAC Encoder，用 shape、contact 和 pose 三条监督路径训练触觉表示，使编码器同时关注物体几何、接触动力学和传感器-物体相对位姿。",
        "构建 8 个代表性任务的 benchmark，覆盖 pose reasoning、shape perception 和 contact-rich interaction，便于系统比较视觉、触觉和多模态策略。",
        "实验显示引入 UniVTAC Encoder 在仿真 benchmark 平均成功率提升 17.1%，真实机器人实验平均成功率相对 vision-only baseline 提升 25%。",
      ],
      implementation: [
        "平台基于 Isaac Lab 和 TacEx/UIPC tactile simulation，支持 GelSight Mini、ViTai GF225 和 Xense WS，并提供统一接口收集 RGB、Depth、Tactile 等观测。",
        "数据生成流程自动完成场景搭建、物体随机化、接触动作执行和多模态数据记录，并可用 cuRobo 规划无碰撞运动。",
        "UniVTAC Encoder 使用 ImageNet 初始化的 ResNet-18 backbone，通过 tactile reconstruction、contact force/marker flow prediction 和 object pose regression 等任务训练。",
        "Benchmark 包含 Lift Bottle、Lift Can、Put Bottle in Shelf、Grasp Classify、Insert Hole、Insert Tube、Insert HDMI 和 Pull Out Key 等任务。",
        "策略评估使用 ACT、ViTAL 和 ACT + UniVTAC 等方法；真实实验在带 ViTai GF225 传感器的 Tianji Marvin 机器人上执行 Insert Tube、Insert USB 和 Bottle Upright。",
      ],
    },
    en: {
      title: "UniVTAC: A Unified Simulation Platform for Visuo-Tactile Manipulation Data Generation, Learning, and Benchmarking",
      ...
      tags: ["Tactile Simulation", "Visuotactile Benchmark", "Representation Learning"],
      mainContent:
        "UniVTAC addresses tactile-data scarcity and fragmented evaluation in visuo-tactile robot learning with a unified simulation platform. It supports three common tactile sensor families, GelSight Mini, ViTai GF225, and Xense WS, and automatically generates RGB, depth, tactile, and action data. On top of this platform, the paper trains a UniVTAC Encoder for tactile-centric visuo-tactile representations and introduces an eight-task benchmark for evaluating tactile-driven manipulation policies.",
      innovations: [
        "It introduces a unified visuo-tactile simulation platform covering multiple sensor types and contact tasks, reducing the cost of tactile data generation and benchmark construction.",
        "Automated manipulation APIs generate controllable contact interactions such as grasping, moving, placing, probing, and rotating for large-scale multimodal synthesis.",
        "The UniVTAC Encoder is trained with shape, contact, and pose supervision pathways, making the representation aware of object geometry, contact dynamics, and sensor-object pose.",
        "The UniVTAC Benchmark contains eight representative tasks across pose reasoning, shape perception, and contact-rich interaction for systematic comparison of vision, tactile, and multimodal policies.",
        "Experiments report a 17.1% average success-rate gain on the simulation benchmark and a 25% real-world improvement over vision-only baselines when using the UniVTAC Encoder.",
      ],
      implementation: [
        "The platform is built on Isaac Lab and TacEx/UIPC tactile simulation, supporting GelSight Mini, ViTai GF225, and Xense WS through unified RGB, depth, and tactile observation APIs.",
        "The data-generation pipeline automates scene setup, object randomization, contact-action execution, and multimodal logging, with cuRobo used for collision-free motion planning.",
        "The UniVTAC Encoder uses an ImageNet-initialized ResNet-18 backbone trained through tactile reconstruction, contact force or marker-flow prediction, and object pose regression.",
        "The benchmark tasks include Lift Bottle, Lift Can, Put Bottle in Shelf, Grasp Classify, Insert Hole, Insert Tube, Insert HDMI, and Pull Out Key.",
        "Policy evaluation compares ACT, ViTAL, and ACT + UniVTAC variants; real-world experiments use a Tianji Marvin robot with ViTai GF225 sensors on Insert Tube, Insert USB, and Bottle Upright.",
      ],
    },
  },
  {
    id: "tac2real",
    categories: ["tactile-simulation"],
    pdf: "https://arxiv.org/pdf/2603.28475",
    project: "https://ningyurichard.github.io/tac2real-project-page/",
    arxiv: "https://arxiv.org/abs/2603.28475",
    year: "2026",
    venue: "arXiv:2603.28475",
    zh: {
      ...
      status: "已整理",
      tags: ["Tactile Simulation", "Online RL", "Zero-Shot Sim-to-Real"],
      mainContent:
        "Tac2Real 研究如何让视觉触觉仿真既有足够物理一致性，又能快到支持在线强化学习。论文提出轻量 GPU 视觉触觉仿真框架，将 PNCG-IPC 软体接触求解器与多节点、多 GPU 并行仿真架构结合，在交互速率下生成 marker displacement fields；同时提出 TacAlign 系统化缩小结构性和随机性 sim-to-real gap，并在接触丰富的 peg insertion 和 nut threading 任务上验证在线 RL 与零样本真实部署。",
      innovations: [
        "把 Preconditioned Nonlinear Conjugate Gradient IPC 引入视觉触觉仿真，在大变形、滑动和旋转接触下比 MPM 类方法更稳定。",
        "采用 plugin-based 架构嵌入现有物理引擎，在 physics step 中提取相对位姿并生成触觉 marker displacement，支持在线 RL 闭环训练。",
        "设计多节点多 GPU 高吞吐并行架构，在 16 张 RTX 4090 和 4090 个环境上达到 4465 FPS，兼顾仿真规模与物理可信度。",
        "提出 TacAlign，把 robot control alignment、baseline IPC 参数标定、task-based 参数标定和随机化组合起来，同时处理结构性和随机性 domain gap。",
        "真实 peg insertion 零样本迁移成功率达到 91.7%，显著高于同设置下 TacSL 的真实部署结果。",
      ],
      implementation: [
        "视觉触觉仿真使用 PNCG-IPC 处理 gel/marker 接触形变，并输出可作为策略观测的 marker displacement field。",
        "Tac2Real 与机器人仿真器并行运行，RL observation 由机器人状态、任务状态和 tactile displacement 组合构成。",
        "仿真验证使用 cube、cylinder、moon、triangle 等压头进行 press、slide、rotate 接触，比较 Tac2Real、Tacchi 和 TacSL 在小/大形变下的真实一致性与数值稳定性。",
        "在线 RL 任务包括 random-orientation peg insertion 和 random-orientation nut threading，对比 Tac2Real、TacSL、Tacchi 与无触觉反馈的学习曲线。",
        "真实部署中在 0 度、15 度和 -15 度等 peg 初始偏差下测试零样本插入，消融显示 task-based calibration 和 robot control alignment 对缩小真实差距很关键。",
      ],
    },
    en: {
      title: "Tac2Real: Reliable and GPU Visuotactile Simulation for Online Reinforcement Learning and Zero-Shot Real-World Deployment",
      ...
      tags: ["Tactile Simulation", "Online RL", "Zero-Shot Sim-to-Real"],
      mainContent:
        "Tac2Real studies how to make visuotactile simulation physically reliable while fast enough for online reinforcement learning. It combines a PNCG-IPC soft-contact solver with a multi-node, multi-GPU parallel simulation architecture to generate marker displacement fields at interactive rates. The paper also proposes TacAlign to systematically reduce both structured and stochastic sim-to-real gaps, then evaluates online RL and zero-shot real deployment on contact-rich peg insertion and nut threading tasks.",
      innovations: [
        "It brings Preconditioned Nonlinear Conjugate Gradient IPC into visuotactile simulation, improving stability under large deformation, sliding, and rotational contact compared with MPM-style methods.",
        "A plugin-based architecture integrates the tactile simulator into existing physics engines, extracting relative poses during physics steps and producing marker displacement observations for closed-loop online RL.",
        "The multi-node, multi-GPU throughput design reaches 4465 FPS with 4090 environments on 16 RTX 4090 GPUs, balancing scale with physical fidelity.",
        "TacAlign combines robot-control alignment, baseline IPC parameter calibration, task-based parameter calibration, and randomization to handle structured and stochastic domain gaps.",
        "Zero-shot real-world peg insertion reaches a 91.7% success rate, substantially outperforming TacSL under the same real deployment setting.",
      ],
      implementation: [
        "The visuotactile simulator uses PNCG-IPC to model gel and marker contact deformation, outputting marker displacement fields as policy observations.",
        "Tac2Real runs alongside the robot simulator, with RL observations combining robot state, task state, and tactile displacements.",
        "Simulation validation uses cube, cylinder, moon, and triangle indenters under pressing, sliding, and rotating contacts, comparing Tac2Real, Tacchi, and TacSL for real-world consistency and numerical stability.",
        "Online RL tasks include random-orientation peg insertion and random-orientation nut threading, comparing Tac2Real, TacSL, Tacchi, and tactile-free policies through learning curves.",
        "Real deployment tests zero-shot insertion under 0-degree, 15-degree, and -15-degree peg offsets, with ablations showing that task-based calibration and robot-control alignment are critical for narrowing the reality gap.",
      ],
    },
  },
  {
    id: "etac",
    categories: ["tactile-simulation"],
    pdf: "https://arxiv.org/pdf/2604.20295",
    project: "https://github.com/Crxus/ETac",
    arxiv: "https://arxiv.org/abs/2604.20295",
    year: "2026",
    venue: "ICRA 2026 / arXiv:2604.20295",
    zh: {
      ...
      status: "已整理",
      tags: ["Tactile Simulation", "Dexterous Manipulation", "Deformation Propagation"],
      mainContent:
        "ETac 研究如何在触觉仿真中同时获得软体接触的高保真和强化学习所需的高效率。它用轻量级数据驱动 deformation propagation model 近似弹性体软体交互，在保持接近 FEM 形变质量的同时支持大规模并行训练，并把大面积触觉反馈用于灵巧手 blind grasping 策略学习。",
      innovations: [
        "用学习型形变传播模型替代昂贵 FEM，全局估计弹性体表面形变，兼顾效率和高保真。",
        "模型同时考虑线性和非线性传播效应，可处理各向异性、曲面传感器和大面积手部触觉覆盖。",
        "作为触觉仿真后端时，表面形变估计接近 FEM，同时更适合接入 Isaac Gym 等并行 RL 环境。",
        "在单张 RTX 4090 上支持 4096 个并行环境，报告总吞吐约 869 FPS，显著缓解触觉 RL 的仿真瓶颈。",
        "通过 blind grasping 任务展示大面积触觉输入能提升灵巧手抓取成功率，说明触觉仿真不只是渲染模块，也能直接服务策略学习。",
      ],
      implementation: [
        "先用 FEM 生成弹性体接触数据，再训练 propagation model 学习 contact nodes 到 surface deformation 的传播关系。",
        "ETac 用粒子/表面节点表示触觉弹性体，计算接触区域后通过传播模型估计完整表面位移场。",
        "框架包含触觉仿真核心、Propagation_calib 标定流程和 BlindGrasping 训练环境，可作为灵巧手触觉传感器后端。",
        "策略训练把触觉表面形变场与机器人本体状态组合成观测，用 PPO 在大规模并行环境中学习 blind grasping。",
        "实验比较 FEM、Taxim、TacSL 等方法的形变质量、吞吐和内存开销，并评估不同触觉输入对抓取策略的影响。",
      ],
    },
    en: {
      title: "ETac: A Lightweight and Efficient Tactile Simulation Framework for Learning Dexterous Manipulation",
      ...
      tags: ["Tactile Simulation", "Dexterous Manipulation", "Deformation Propagation"],
      mainContent:
        "ETac studies how tactile simulation can provide both high-fidelity soft-body contact and the efficiency needed for reinforcement learning. It uses a lightweight data-driven deformation propagation model to approximate elastomer interactions, producing FEM-like surface deformation estimates while supporting large-scale parallel training for tactile blind grasping with dexterous hands.",
      innovations: [
        "A learned deformation propagation model replaces expensive FEM and estimates global elastomer surface deformation with a better fidelity-efficiency tradeoff.",
        "The model captures both linear and nonlinear propagation effects, supporting anisotropy, curved sensors, and large-area tactile coverage on robotic hands.",
        "As a tactile simulation backend, ETac produces deformation estimates comparable to FEM while being practical inside parallel RL environments such as Isaac Gym.",
        "On a single RTX 4090, it supports 4096 parallel environments with a reported total throughput of about 869 FPS, reducing a major bottleneck for tactile RL.",
        "Blind grasping experiments show that large-area tactile feedback improves dexterous manipulation performance, making ETac useful as a policy-learning backend rather than only a rendering module.",
      ],
      implementation: [
        "FEM-generated contact data is used first to train the propagation model from contact nodes to full surface deformation.",
        "ETac represents the tactile elastomer with particles or surface nodes, detects contact regions, and predicts full displacement fields through the propagation model.",
        "The repository includes the tactile simulation core, a Propagation_calib calibration workflow, and a BlindGrasping training environment for dexterous-hand tactile sensing.",
        "Policy learning combines tactile surface deformation fields with robot proprioception and trains a PPO blind-grasping policy in massively parallel environments.",
        "Experiments compare deformation quality, throughput, and memory against FEM, Taxim, and TacSL, then evaluate the effect of tactile observations on grasping policies.",
      ],
    },
  },
  {
    ...
    zh: {
      ...
      status: "已整理",
      tags: ["Tactile Simulation", "Differentiable Physics", "FEM"],
      mainContent:
        "DiffTactile 研究如何构建物理驱动、可微分的触觉仿真系统，用密集且物理准确的触觉反馈支持接触丰富操作。它把 FEM 触觉弹性体、多材料物体仿真、penalty-based contact dynamics 和可学习光学响应模块结合起来，使触觉仿真不仅能输出图像/形变，也能通过梯度优化材料参数和辅助策略学习。",
      innovations: [
        "强调 physics-based differentiable tactile simulation，而不是只用刚体接触或局部几何近似生成触觉信号。",
        "用 FEM soft-body model 表示 sensing elastomer，支持更高保真的应力、形变和接触建模。",
        "多材料仿真器支持 elastic、elastoplastic、cable 等不同物体类型，覆盖更丰富的接触模式。",
        "可微分特性允许用真实数据做物理参数优化，也能用于触觉辅助抓取和接触操作技能学习。",
        "引入 pixel-based neural module 估计触觉传感器光学响应，把物理接触形变转成视觉触觉图像。",
      ],
      implementation: [
        "触觉弹性体由 FEM soft-body solver 更新，物体和传感器之间通过 penalty-based contact model 计算接触力和形变。",
        "系统在同一个可微仿真框架中处理软体传感器、多材料物体和接触动力学，便于端到端梯度传播。",
        "光学模块根据接触导致的弹性体表面变化预测像素级触觉图像响应，连接物理形变和传感器观测。",
        "论文展示 surface following、box opening、cable manipulation、object reposing 等接触丰富任务。",
        "可通过真实观测反向优化仿真物理参数，用于缩小 tactile sim-to-real gap，并训练触觉辅助操作策略。",
      ],
    },
    en: {
      title: "DIFFTACTILE: A Physics-based Differentiable Tactile Simulator for Contact-rich Robotic Manipulation",
      ...
      tags: ["Tactile Simulation", "Differentiable Physics", "FEM"],
      mainContent:
        "DiffTactile builds a physics-based, differentiable tactile simulation system for dense and physically accurate tactile feedback in contact-rich manipulation. It combines an FEM sensing elastomer, a multi-material object simulator, penalty-based contact dynamics, and a learned optical-response module, allowing tactile simulation to support both realistic observations and gradient-based parameter or policy optimization.",
      innovations: [
        "It emphasizes physics-based differentiable tactile simulation instead of relying only on rigid-body contact or local geometric approximations.",
        "An FEM soft-body model represents the sensing elastomer, improving fidelity for stress, deformation, and contact modeling.",
        "The multi-material simulator supports elastic, elastoplastic, and cable-like objects, covering a richer set of contact modes.",
        "Differentiability enables physical-parameter refinement from real data and efficient learning of tactile-assisted grasping and manipulation skills.",
        "A pixel-based neural module maps contact-induced elastomer deformation into the optical response of a visuotactile sensor.",
      ],
      implementation: [
        "The tactile elastomer is updated by an FEM soft-body solver, while a penalty-based contact model computes interaction forces and deformation.",
        "Soft sensing, multi-material objects, and contact dynamics live in one differentiable framework so gradients can propagate end to end.",
        "The optical module predicts pixel-level tactile images from the simulated surface changes, connecting physical deformation to sensor observations.",
        "The paper demonstrates contact-rich tasks including surface following, box opening, cable manipulation, and object reposing.",
        "Real observations can be used to optimize simulator physical parameters and narrow the tactile sim-to-real gap while training tactile-assisted policies.",
      ],
    },
  },
  {
    id: "beyond-flat-gelsight",
    categories: ["tactile-simulation"],
    pdf: "https://arxiv.org/pdf/2305.12605",
    project: "https://danfergo.github.io/geltip-sim/",
    arxiv: "https://arxiv.org/abs/2305.12605",
    year: "2023",
    venue: "RSS 2023 / arXiv:2305.12605",
    zh: {
      title: "Beyond Flat GelSight Sensors: Simulation of Optical Tactile Sensors of Complex Morphologies for Sim2Real Learning",
      authors: "Daniel Fernandes Gomes, Paolo Paoletti, Shan Luo",
      status: "已整理",
      tags: ["Tactile Simulation", "GelSight", "Curved Sensor"],
      mainContent:
        "Beyond Flat GelSight Sensors 研究如何把传统平面 GelSight 光学触觉仿真扩展到复杂形态传感器，尤其是手指形 GelTip。核心问题是曲面触觉膜中的光传播不再能简单看作平面常量光场，因此论文用传感器网格上的 geodesic path 近似光在曲面膜中的传播，并用真实 GelTip 数据验证合成触觉图像。",
      innovations: [
        "突破已有 GelSight 仿真主要面向平面传感器的限制，使曲面/复杂形态视觉触觉传感器也能用于 Sim2Real 数据生成。",
        "把曲面膜内光传播建模为沿传感器 mesh 的 geodesic path，而不是简单线性路径或平面光场。",
        "方法由传感器网格参数化，原则上可迁移到任意形态的 optical tactile sensor。",
        "结合 geodesic illumination 和真实传感器背景图像，可生成更接近真实 GelTip 的合成触觉 RGB。",
        "该仿真框架既能服务已有复杂形态传感器，也能在真实制作前评估新形态触觉传感器设计。",
      ],
      implementation: [
        "先离线根据传感器 mesh 计算不同 light field，包括 linear、plane、geodesic 和 transport 等方案。",
        "在线阶段从仿真环境得到 depth map，进行平滑以近似弹性体形变，再映射到相机坐标下的点云。",
        "使用预计算光场和 Phong reflection model 生成触觉 RGB 图像，并可叠加真实传感器背景以提升外观一致性。",
        "数据采集使用真实 GelTip 与仿真深度环境，对 8 个 3D 打印物体做重复 tapping，建立真实 RGB 与合成深度/图像对比。",
        "实验比较不同光场的 SSIM、分类和接触定位表现，发现 geodesic/transport 与真实背景结合能更好匹配曲面传感器观测。",
      ],
    },
    en: {
      title: "Beyond Flat GelSight Sensors: Simulation of Optical Tactile Sensors of Complex Morphologies for Sim2Real Learning",
      ...
      tags: ["Tactile Simulation", "GelSight", "Curved Sensor"],
      mainContent:
        "Beyond Flat GelSight Sensors extends optical tactile simulation from flat GelSight-style sensors to complex sensor morphologies, especially the finger-shaped GelTip. The key issue is that light propagation through a curved tactile membrane cannot be treated as a simple flat constant field, so the paper approximates light travel with geodesic paths over the sensor mesh and validates synthetic tactile images against real GelTip data.",
      innovations: [
        "It removes the flat-sensor limitation of prior GelSight simulation methods, enabling curved or complex-morphology visuotactile sensors for Sim2Real data generation.",
        "Light propagation through the curved membrane is modeled with geodesic paths on the sensor mesh rather than simple linear rays or flat light fields.",
        "The method is parameterized by the sensor mesh and can in principle apply to optical tactile sensors of arbitrary morphology.",
        "Combining geodesic illumination with a real sensor background image produces synthetic tactile RGB closer to real GelTip observations.",
        "The simulator supports existing complex tactile sensors and can also help evaluate new sensor morphologies before fabrication.",
      ],
      implementation: [
        "Different light fields, including linear, plane, geodesic, and transport variants, are precomputed offline from the sensor mesh.",
        "At runtime, the simulator obtains a depth map, smooths it to mimic elastomer deformation, and maps it into a camera-frame point cloud.",
        "The precomputed light field and a Phong reflection model generate tactile RGB images, optionally using real sensor backgrounds for better appearance matching.",
        "The validation dataset pairs real GelTip RGB images with corresponding simulated depth/image data by tapping eight 3D-printed objects.",
        "Experiments compare SSIM, classification, and contact localization across light-field choices, finding that geodesic or transport fields with real backgrounds best match curved-sensor observations.",
      ],
    },
  },
  {
    id: "tacto",
    categories: ["tactile-simulation"],
    pdf: "https://arxiv.org/pdf/2012.08456",
    project: "https://github.com/facebookresearch/tacto",
    arxiv: "https://arxiv.org/abs/2012.08456",
    year: "2022",
    venue: "IEEE RA-L 2022 / arXiv:2012.08456",
    zh: {
      title: "TACTO: A Fast, Flexible, and Open-source Simulator for High-Resolution Vision-based Tactile Sensors",
      authors: "Shaoxiong Wang, Mike Lambeta, Po-Wei Chou, Roberto Calandra",
      status: "已整理",
      tags: ["Tactile Simulation", "Vision-based Tactile Sensor", "Open-source"],
      mainContent:
        "TACTO 是面向高分辨率视觉触觉传感器的快速、灵活、开源仿真器，目标是在通用机器人仿真和学习流程中高效生成类似 DIGIT、OmniTact 等传感器的触觉图像。它侧重把触觉图像渲染做成可配置、可扩展的模块，让研究者能在物理引擎之外获得高速视觉触觉观测，并用于感知、控制和 Sim2Real 实验。",
      innovations: [
        "提出开源视觉触觉仿真器，把高分辨率 tactile image rendering 做成易配置工具，降低触觉仿真使用门槛。",
        "支持多种 vision-based tactile sensors，包括 DIGIT 和 OmniTact，可通过传感器几何、相机和光照配置适配不同硬件。",
        "强调速度和工程可用性，可在每秒数百帧级别渲染触觉读数，适合大规模数据生成和机器人学习。",
        "通过 100 万次抓取数据训练 grasp stability prediction，展示触觉仿真可服务大规模触觉感知学习。",
        "在 marble manipulation 和 Sim2Real proof-of-concept 中验证 TACTO 可用于闭环控制和真实迁移探索。",
      ],
      implementation: [
        "TACTO 通常与 PyBullet 等刚体物理引擎结合：物理引擎负责物体运动和接触几何，TACTO 负责根据接触区域渲染视觉触觉图像。",
        "渲染流程使用传感器 mesh、相机参数、光照和物体几何，生成高分辨率 RGB tactile readings。",
        "传感器配置文件定义 DIGIT、OmniTact 等不同 tactile sensor 的视角、尺寸、光照和表面属性。",
        "示例任务包括抓取稳定性预测和 marble manipulation，用来测试触觉图像在感知和控制中的作用。",
        "TACTO 更偏工程化高速 optical tactile rendering，不是 FEM/IPC 级别的软体接触求解器，因此适合快速原型、数据生成和基准实验。",
      ],
    },
    en: {
      title: "TACTO: A Fast, Flexible, and Open-source Simulator for High-Resolution Vision-based Tactile Sensors",
      ...
      tags: ["Tactile Simulation", "Vision-based Tactile Sensor", "Open-source"],
      mainContent:
        "TACTO is a fast, flexible, open-source simulator for high-resolution vision-based tactile sensors. It is designed to generate realistic tactile images for sensors such as DIGIT and OmniTact inside robotics and learning workflows. Rather than being a full soft-body solver, it focuses on configurable and efficient tactile image rendering that can support perception, control, and Sim2Real experiments.",
      innovations: [
        "It provides an open-source visual tactile simulator with configurable high-resolution tactile image rendering, lowering the barrier to using touch simulation.",
        "The simulator supports multiple vision-based tactile sensors, including DIGIT and OmniTact, through configurable sensor geometry, cameras, and lighting.",
        "TACTO emphasizes speed and usability, rendering tactile readings at hundreds of frames per second for large-scale data generation and robot learning.",
        "A grasp-stability prediction experiment trained from one million grasps demonstrates its value for large-scale tactile perception learning.",
        "Marble manipulation and a Sim2Real proof of concept show that TACTO can support closed-loop control and real-world transfer studies.",
      ],
      implementation: [
        "TACTO is commonly paired with rigid-body engines such as PyBullet: the physics engine handles motion and contact geometry, while TACTO renders tactile images from the contact region.",
        "The rendering pipeline uses sensor meshes, camera parameters, lighting, and object geometry to generate high-resolution RGB tactile readings.",
        "Sensor configuration files specify viewpoints, sizes, lighting, and surface properties for devices such as DIGIT and OmniTact.",
        "Example tasks include grasp stability prediction and marble manipulation to test tactile images for perception and control.",
        "TACTO is best understood as fast engineering-oriented optical tactile rendering rather than an FEM or IPC-level soft-contact solver, making it useful for prototyping, data generation, and benchmarks.",
      ],
    },
  },
  {
